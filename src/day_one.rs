...
pub fn calculate_final_frequency(frequencies: Vec<String>) -> i32 {
    let mut sum = 0;
...
pub fn find_repeated_frequency(frequencies: Vec<String>) -> i32 {
    ...
    fn test_find_repeated_frequency() {
        ...
        for test in &tests {
            let test_input = test
                ...
                .map(|&x| x.to_owned())
                .collect::<Vec<String>>();
            assert_eq!(test.expected, find_repeated_frequency(test_input));
        }
    }
}
